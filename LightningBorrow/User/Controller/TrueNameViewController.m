...
@interface TrueNameViewController () <UITableViewDelegate, UITableViewDataSource>
...
- (void)viewDidLoad {
    ...
    self.dataArray = @[@"姓名", @"身份证号码", @"现住地址", @"手机号码", @"验证码"];
    [self initUI];
...
- (void)sureAction
{
    ...
    if (![self checkUserID:cell2.subLabel.text]) {
        [SVProgressHUD showErrorWithStatus:@"请输入正确的身份证号"];
        return;
    }
    if ([cell3.subLabel.text isEqualToString:@"请输入现住地址"]) {
        [SVProgressHUD showErrorWithStatus:@"现住地址不能为空"];
        return;
    }
